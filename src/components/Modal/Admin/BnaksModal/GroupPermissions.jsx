import {
  useBanksBlockListUserGroup,
  useToggleBlockBankUserGroup,
} from "api/admin/bank";
...
export default function GroupPermissions({ isOpen, toggle, bankId }) {
  ...
  } = useBanksBlockListUserGroup(bankId, searchValue);
  ...
  return (
    <ActionModal
      ...
      close={toggle}
      size="small"
      title={`تعديل صلاحيات  مجموعات الزبائن لبنك`}
    >
...
function ToggleBlockCategoryButton({ currentState, roleId, bankId }) {
  const { mutate: toggleStatus, isPending: toggleStatusIsPending } =
    useToggleBlockBankUserGroup(bankId);
  return (
    <button
      className={`btn ${
