import {
  useCategoryBlockList,
  useToggleBlockCategory,
  useToggleBlockCategoryAll,
...
export default function CustomerPermissions({ isOpen, toggle, categoryId }) {
  ...
  } = useCategoryBlockList(categoryId, searchValue);
  ...
  return (
    <ActionModal
      ...
      close={toggle}
      size="small"
      title={`تعديل صلاحيات الزبائن لتصنيف`}
    >
...
function ToggleBlockCategoryButton({ currentState, userId, categoryId }) {
  const { mutate: toggleStatus, isPending: toggleStatusIsPending } =
    useToggleBlockCategory(categoryId);
  return (
    <button
      className={`btn ${
