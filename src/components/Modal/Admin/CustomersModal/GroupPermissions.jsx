import {
  useCategoryBlockListUserGroup,
  useToggleBlockCategoryUserGroup,
...
export default function GroupPermissions({ isOpen, toggle, categoryId }) {
  ...
  } = useCategoryBlockListUserGroup(categoryId, searchValue);

  // const { mutate: onStatusAll, isPending: onStatusIsPending } =
  //   useToggleBlockCategoryAll(categoryId);
  ...
  return (
    <ActionModal
      ...
      close={toggle}
      size="small"
      title={`تعديل صلاحيات  مجموعات الزبائن لتصنيف`}
    >
...
function ToggleBlockCategoryButton({ currentState, roleId, categoryId }) {
  const { mutate: toggleStatus, isPending: toggleStatusIsPending } =
    useToggleBlockCategoryUserGroup(categoryId);
  return (
    <button
      className={`btn ${
        !currentState ? "btn-success" : "btn-warning"
      } text-xs_ !p-1.5`}
      onClick={() => toggleStatus({ roleId })}
