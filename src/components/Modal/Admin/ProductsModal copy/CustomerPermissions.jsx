...
import {
  useProductsBlockList,
  useToggleBlockProduct,
  useToggleBlockProductAll,
} from "api/admin/product";
import { FaBan, FaRegCheckCircle } from "react-icons/fa";

export default function CustomerPermissions({ isOpen, toggle, productId }) {
  ...
  } = useProductsBlockList(productId, searchValue);

  const { mutate: onStatusAll, isPending: onStatusIsPending } =
    useToggleBlockProductAll(productId);
  ...
  return (
    <ActionModal
      ...
      close={toggle}
      size="small"
      title={`تعديل صلاحيات الزبائن لمنتج`}
    >
...
function ToggleBlockProductButton({ currentState, userId, productId }) {
  const { mutate: toggleStatus, isPending: toggleStatusIsPending } =
    useToggleBlockProduct(productId);
  return (
    <button
      className={`btn ${
