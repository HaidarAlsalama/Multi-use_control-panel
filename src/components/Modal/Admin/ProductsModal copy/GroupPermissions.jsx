import {
  useProductBlockListUserGroup,
  useToggleBlockProductUserGroup,
...
export default function GroupPermissions({ isOpen, toggle, categoryId }) {
  ...
  } = useProductBlockListUserGroup(categoryId, searchValue);
  ...
  return (
    <ActionModal
      ...
      close={toggle}
      size="small"
      title={`تعديل صلاحيات  مجموعات الزبائن لمنتج`}
    >
...
function ToggleBlockCategoryButton({ currentState, roleId, categoryId }) {
  const { mutate: toggleStatus, isPending: toggleStatusIsPending } =
    useToggleBlockProductUserGroup(categoryId);
  return (
    <button
      className={`btn ${
