import { useCategoriesOrder } from "api/admin/category";
import { useProductsForOrder } from "api/admin/product";
...
export default function OrderProducts({ isOpen, toggle, idCategory }) {
  ...
  } = useProductsForOrder(idCategory);

  const {
    mutate: orderIt,
    isPending: orderItIsPending,
    isSuccess: orderItIsSuccess,
  } = useCategoriesOrder("product");
  ...
  return (
    ...
    >
      {productsIsLoading ? (
        <Spinner />
      ) : !productsIsError && items.length > 0 ? (
