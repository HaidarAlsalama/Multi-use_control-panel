...
export default function EditRole({ isOpen, toggle, roleId }) {
  ...
  } = useAllCateoriesCustomize(true);

  useEffect(() => {
    if (rolesIsSuccess) {
      setSelectedPermissions(roles.data.permissions.map((role) => role.name));
      reset({ name: roles?.data.name });
    }
  }, [roles]);

  useEffect(() => {
    if (categoriesIsSuccess) {
      setSelectedCategories(roles?.data?.categories.map((cat) => cat.id) || []);
    }
  }, [categories, categoriesIsIsLoading]);

  const {
    mutate: editRole,
    isPending: editRoleIsPending,
  ...
  return (
    <ActionModal open={isOpen} close={toggle} size="small" title={"تعديل دور"}>
      {!rolesIsIsLoading ? (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <button
            ...
            disabled={editRoleIsPending}
          >
            {editRoleIsPending ? <Spinner sm /> : "تعديل"}
          </button>
        </form>
      ) : (
        <Spinner />
