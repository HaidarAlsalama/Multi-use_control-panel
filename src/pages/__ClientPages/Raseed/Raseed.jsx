...
const Raseed = () => {
  ...
  const { balance, currency } = useSelector((state) => state.balance);
  ...
  return (
      ...
      >
        ...
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black">رصيد مفرق</h1>
    ...
    dark:bg-black/20 dark:border-white/10"
              ...
              {/* {transferType === "prepaid" ||
              (transferType === "cash" && operator == "mtn") ? (
                <div className="relative w-2/3 flex flex-col gap-1">
                  <label>الكمية</label>
                  <select
                    value={amount}
                    required
                    onChange={(e) => handelAmountChange(e.target.value)}
                    className="w-full px-5 py-4 rounded-xl border appearance-none
            bg-white border-gray-200 outline-none 
            dark:bg-black/20 dark:border-white/10"
                  >
                    <option value="">اختر الكمية</option>
                    {quantityField?.options?.split("-").map((v) => (
                      <option key={v} value={v}>
                        {v} ل.س
                      </option>
                    ))}
                  </select>
                  <ChevronDown className="absolute left-4 top-2/3 -translate-y-1/2 opacity-30" />
                </div>
              ) : (
                <div className="relative w-2/3 flex flex-col gap-1">
                  <label>الكمية</label>
        ...
        text-gray-700 dark:text-white font-bold"
          ...
          <div className="flex gap-3 p-4 rounded-xl bg-white/5 border border-white/5 items-center">
            ...
            <p className="text-[10px] opacity-40 font-bold">
              هذه العمليه مؤتمتة بالكامل لايمكن الرجوع او التعديل يرجى التاكد من
              البيانات قبل الارسال.
            </p>
          </div>
