import { useBankNotice } from "api/admin/bankNotice";
...
export default function BankNotice() {
  ...
  } = useBankNotice(limit, pageNumber, searchValue, status, fromDate, toDate);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `الاشعارات البنكية.xlsx`);
  };
              ...
              chargeStatements?.data?.chargeStatements.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-2">
                    <h1 className="break-all min-w-28">{item.notify_number}</h1>
                  </td>
